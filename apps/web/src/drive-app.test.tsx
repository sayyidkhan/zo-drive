...
describe("DriveApp", () => {
  ...
  it("documents separate GUI and CLI workflows", () => {
    ...
    expect(screen.getByRole("heading", { name: "GUI version 1.25.0" })).toBeInTheDocument();
      ...
      expect(screen.getByText("ZominAI documentation · v1.1.0")).toBeInTheDocument();
      expect(screen.getAllByRole("link", { name: "ZominAI" })[0]).toHaveAttribute("aria-current", "page");
      expect(screen.getByRole("heading", { name: "ZominAI version 1.1.0" })).toBeInTheDocument();
  ...
  it("shows storage usage, folders, and files supplied by the shared SDK", async () => {
    ...
    expect(screen.getByRole("heading", { name: "Use ZominAI from every signed-in device." })).toBeInTheDocument();
    ...
    expect(screen.getByRole("link", { name: /Bonsai 8B model & licence/ })).toHaveAttribute("href", "https://huggingface.co/prism-ml/Bonsai-8B-gguf");
    ...
    expect(screen.getByRole("button", { name: "ZominAI menu: Verify service" })).toBeInTheDocument();
    expect(screen.queryByRole("button", { name: "ZominAI menu: Talk to ZominAI" })).not.toBeInTheDocument();
    expect(screen.getByRole("button", { name: "ZominAI menu: Zo Computer setup" })).toBeInTheDocument();
    expect(screen.getByRole("button", { name: "ZominAI menu: Service settings" })).toBeInTheDocument();
    expect(screen.getByRole("button", { name: "ZominAI menu: Uninstall ZominAI" })).toBeInTheDocument();
    fireEvent.click(screen.getByRole("button", { name: "ZominAI menu: Zo Computer setup" }));
    ...
    expect(screen.getByRole("link", { name: /Bonsai 8B model & licence/ })).toHaveAttribute("href", "https://huggingface.co/prism-ml/Bonsai-8B-gguf");
    expect(screen.getByRole("link", { name: /Runtime installation docs/ })).toHaveAttribute("href", "https://github.com/ggml-org/llama.cpp/blob/master/docs/install.md");
    fireEvent.click(screen.getByRole("button", { name: "ZominAI menu: Service settings" }));
