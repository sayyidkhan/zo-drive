...
describe("DriveApp", () => {
  it("shows a public landing page with Drive and documentation entry points", () => {
    ...
    expect(document.querySelector(".landing-particle-field")).toBeInTheDocument();
    expect(document.querySelector(".landing-particle-field")).toHaveClass("z-0");
    expect(screen.queryByText("Data stays on your Zo")).not.toBeInTheDocument();
    expect(screen.queryByText("Folder-preserving uploads")).not.toBeInTheDocument();
    expect(screen.queryByText("GUI & CLI Access")).not.toBeInTheDocument();
    ...
    expect(screen.getByRole("heading", { name: "Work locally. Keep Zo authoritative." })).toBeInTheDocument();
    expect(screen.getByLabelText("Remote local-machine access")).toHaveTextContent("Your local machine");
    expect(screen.getByLabelText("Animated remote CLI session")).toHaveAttribute("data-animated-terminal", "true");
    expect(screen.getByLabelText("Animated remote CLI session")).toHaveTextContent("Ready for the next job");
    expect(screen.getByRole("link", { name: "Read the remote CLI guide" })).toHaveAttribute("href", expect.stringContaining("?docs=1&mode=cli"));
    expect(screen.getByText("Built-in tools")).toBeInTheDocument();
    expect(screen.getByText("6 tools")).toBeInTheDocument();
    expect(screen.getByRole("heading", { name: "Six focused products. One calm system." })).toBeInTheDocument();
    ...
    expect(screen.getAllByRole("tab", { name: "Cost comparison" })).toHaveLength(1);
    fireEvent.click(screen.getByLabelText("Select Zo Paste"));
    fireEvent.click(screen.getByRole("tab", { name: "Cost comparison" }));
    expect(screen.getByText("Existing SaaS")).toBeInTheDocument();
    expect(screen.getByLabelText("Zo Paste monthly cost saving")).toHaveTextContent("Avoid another subscription");
    fireEvent.click(screen.getByLabelText("Select Zo Transfer"));
    fireEvent.click(screen.getByRole("tab", { name: "Cost comparison" }));
    expect(screen.getByLabelText("Zo Transfer monthly cost saving")).toHaveTextContent("Save US$25/month");
    expect(within(screen.getByLabelText("Zo Transfer feature views")).getByText("WeTransfer Ultimate")).toBeInTheDocument();
    fireEvent.click(screen.getByLabelText("Select Zo Paste"));
    expect(screen.getByText("launch-notes.md")).toBeInTheDocument();
    fireEvent.click(screen.getByLabelText("Select Zo Transfer"));
    expect(screen.getByText("launch-brief.pdf")).toBeInTheDocument();
    fireEvent.click(screen.getByLabelText("Select Zo Functions"));
    expect(within(screen.getByLabelText("Zo Functions feature views")).getByText("Every Monday, 09:00 UTC")).toBeInTheDocument();
    expect(screen.getByLabelText("Next feature view in 5 seconds")).toBeInTheDocument();
    fireEvent.click(screen.getByLabelText("Select Zo Databases"));
    expect(screen.getByText("product.sqlite")).toBeInTheDocument();
    fireEvent.click(screen.getByLabelText("Select Zo Shared Drives"));
    expect(screen.getByText("Amira")).toBeInTheDocument();
    fireEvent.click(screen.getByLabelText("Select ZominAI"));
    expect(screen.getByText("Read-only Drive tools")).toBeInTheDocument();
    expect(screen.getByRole("heading", { name: "Six subscriptions become one private suite." })).toBeInTheDocument();
    expect(screen.getByLabelText("Fragmented SaaS subscriptions")).toHaveTextContent("US$104+");
    expect(screen.getByLabelText("Fragmented SaaS subscriptions")).toHaveTextContent("US$25 / month");
    expect(screen.getByLabelText("Zo Drive private suite")).toHaveTextContent("US$0");
    expect(screen.getByLabelText("Zo Drive private suite")).toHaveTextContent("US$0");
    expect(screen.queryByText("Each comparison above is explained in the matching product section.")).not.toBeInTheDocument();
    expect(screen.getByRole("heading", { name: "The workspace you own. The work you can move." })).toBeInTheDocument();
    expect(screen.getByRole("link", { name: "Open Zo Drive" })).toHaveAttribute("href", expect.stringContaining("?login=1"));
    expect(screen.getByRole("link", { name: "See how it works" })).toHaveAttribute("href", "#killer-features");
  });

  it("serves the Neumorphic landing concept at landing-page-2", () => {
    window.history.pushState({}, "", "/landing-page-2");
    render(<DriveApp />);

    expect(screen.getByRole("heading", { name: "Your cloud has a home." })).toBeInTheDocument();
    expect(screen.getByRole("heading", { name: "Six controls. One console." })).toBeInTheDocument();
    expect(screen.getByRole("heading", { name: "Six bills become one private system." })).toBeInTheDocument();
    expect(screen.getByLabelText("Regular SaaS products")).toHaveTextContent("WeTransfer Ultimate");
    expect(screen.getByLabelText("Zo Drive product suite")).toHaveTextContent("ZominAI");
    expect(screen.getByRole("link", { name: "Open Drive" })).toHaveAttribute("href", expect.stringContaining("?login=1"));
    expect(screen.getByRole("link", { name: "Open Zo Paste" })).toHaveAttribute("href", expect.stringContaining("section=pastes"));
  });

  it("keeps the Zo Remix hero above the product-platform landing at landing-page-3", () => {
    window.history.pushState({}, "", "/landing-page-3");
    render(<DriveApp />);

    expect(screen.getByRole("heading", { name: "Your cloud, under your control." })).toBeInTheDocument();
    const whyZoHeading = screen.getByRole("heading", { name: "A cloud should increase your agency, not your dependency." });
    const interfaceHeading = screen.getByRole("heading", { name: "One Drive. Two ways to work." });
    const pricingHeading = screen.getByRole("heading", { name: "Six subscriptions become one private suite." });
    const productHeading = screen.getByRole("heading", { name: "One home. Six focused tools." });
    expect(whyZoHeading.compareDocumentPosition(interfaceHeading) & Node.DOCUMENT_POSITION_FOLLOWING).toBeTruthy();
    expect(interfaceHeading.compareDocumentPosition(productHeading) & Node.DOCUMENT_POSITION_FOLLOWING).toBeTruthy();
    expect(productHeading.compareDocumentPosition(pricingHeading) & Node.DOCUMENT_POSITION_FOLLOWING).toBeTruthy();
    expect(screen.getByRole("link", { name: "Explore GUI" })).toHaveAttribute("href", expect.stringContaining("mode=gui"));
    expect(screen.getByRole("link", { name: "Explore CLI" })).toHaveAttribute("href", expect.stringContaining("mode=cli"));
    expect(screen.getByRole("article", { name: "Fragmented SaaS pricing" })).toHaveTextContent("US$104+ / month");
    expect(screen.getByRole("article", { name: "Zo product pricing" })).toHaveTextContent("US$0 extra / feature");
    expect(screen.getByRole("heading", { name: "One home. Six focused tools." })).toBeInTheDocument();
    expect(screen.getByRole("heading", { name: "Share the work. Keep the source." })).toBeInTheDocument();
    expect(screen.getByRole("heading", { name: "One system to operate." })).toBeInTheDocument();
    const sectionNavigation = screen.getByRole("navigation", { name: "Zo Drive section navigation" });
    expect(within(sectionNavigation).getByRole("link", { name: "Why Zo" })).toHaveAttribute("href", "#why-zo");
    expect(within(sectionNavigation).getByRole("link", { name: "Products" })).toHaveAttribute("href", "#products");
    expect(within(sectionNavigation).getByRole("link", { name: "Teams" })).toHaveAttribute("href", "#teams");
    expect(within(sectionNavigation).getByRole("link", { name: "Operators" })).toHaveAttribute("href", "#operators");
    expect(screen.getByRole("link", { name: "Open Zo Drive documentation" })).toHaveAttribute("href", expect.stringContaining("docs=1"));
    expect(screen.getByRole("link", { name: "Open Zo Drive from final call to action" })).toHaveClass("lp3-primary-cta");
    expect(screen.getByRole("contentinfo", { name: "Zo Drive footer" })).toBeInTheDocument();
    expect(screen.getByRole("link", { name: /Sign in to Zo Drive/ })).toHaveAttribute("href", expect.stringContaining("login=1"));
    expect(screen.queryByRole("heading", { name: "Enjoy building your own system." })).not.toBeInTheDocument();
    expect(screen.queryByRole("link", { name: "Explore Zo Paste" })).not.toBeInTheDocument();
    expect(screen.getByRole("tab", { name: /Zo Paste/ })).toBeInTheDocument();
    expect(screen.getByText("Markdown and code")).toBeInTheDocument();
    fireEvent.click(screen.getByRole("tab", { name: /Zo Transfer/ }));
    expect(screen.getByText("Passcodes and expiry")).toBeInTheDocument();
    fireEvent.click(screen.getByRole("tab", { name: /ZominAI/ }));
    expect(screen.getByText("Read-only tools")).toBeInTheDocument();
    expect(screen.getByText("Ask your Drive without granting write access")).toBeInTheDocument();
    expect(screen.queryByText("Markdown and code")).not.toBeInTheDocument();
  ...
  it("documents separate GUI and CLI workflows", () => {
    ...
    try {
      ...
      expect(screen.getByRole("heading", { name: "GUI version 1.42.21" })).toBeInTheDocument();
      ...
      expect(screen.getByRole("link", { name: "GUI releases version 1.42.21" })).toHaveAttribute("href", expect.stringContaining("?releases=1&mode=gui"));
      ...
      expect(screen.getByText("Latest: v1.42.21")).toBeInTheDocument();
  ...
  it("shows storage usage, folders, and files supplied by the shared SDK", async () => {
    ...
    const client = {
      ...
      downloadClusterObject: vi.fn().mockResolvedValue(new Response(new Blob(["shared PDF"], { type: "application/pdf" }))),
    ...
    const fetchSpy = vi.spyOn(globalThis, "fetch").mockImplementation(async (input) => {
      ...
      if (String(input).includes("/zominai/warmup")) {
        warmupCalls += 1;
        if (warmupCalls === 1) return await new Promise<Response>((resolve) => { finishFirstWarmup = () => resolve(new Response(JSON.stringify({ model: "Bonsai-8B-Q1_0.gguf", status: "ready" }), { status: 200 })); });
    ...
    expect(screen.getAllByAltText("ZominAI Pegasus")).toHaveLength(1);
    expect(await screen.findByLabelText("ZominAI warm-up status")).toHaveTextContent("Warming up my system");
    expect(screen.getByRole("textbox", { name: "Message ZominAI" })).toBeDisabled();
    ...
    expect(screen.getByRole("textbox", { name: "Message ZominAI" })).toBeEnabled();
    ...
    expect(screen.getByLabelText("ZominAI response metrics")).toHaveTextContent("TPS unavailable");
    ...
    const greetingRequest = JSON.parse(String(firstChatRequest?.[1]?.body)) as { model: string; tool_choice?: string; tools?: unknown[] };
    expect(greetingRequest).toMatchObject({ model: "Bonsai-27B-Q4_K_M.gguf" });
    expect(greetingRequest.tool_choice).toBeUndefined();
    expect(greetingRequest.tools).toBeUndefined();
    ...
    fetchSpy.mockReset();
    fetchSpy
      .mockResolvedValueOnce(new Response(JSON.stringify({ iso: "2026-07-22T15:30:00.000Z", local: "Wednesday, 22 July 2026 at 15:30:00 UTC", timeZone: "UTC" }), { status: 200 }))
      .mockResolvedValueOnce(new Response(JSON.stringify({ choices: [{ message: { content: "The Zo Computer time is 15:30 UTC on 22 July 2026." } }] }), { status: 200 }));
    fireEvent.change(screen.getByRole("textbox", { name: "Message ZominAI" }), { target: { value: "What is the date and time on this machine?" } });
    fireEvent.click(screen.getByRole("button", { name: "Send message to ZominAI" }));
    expect(await screen.findByText("The Zo Computer time is 15:30 UTC on 22 July 2026.")).toBeInTheDocument();
    expect(String(fetchSpy.mock.calls[0]?.[0])).toContain("/zominai/time");
    const timeRequest = JSON.parse(String(fetchSpy.mock.calls[1]?.[1]?.body)) as { messages: Array<{ content: string; role: string }>; tools?: unknown[] };
    expect(timeRequest.tools).toBeUndefined();
    expect(timeRequest.messages[0]?.content).toContain("2026-07-22T15:30:00.000Z");
    fetchSpy.mockResolvedValueOnce(new Response(JSON.stringify({ choices: [{ message: { content: "You asked for the date and time on this machine." } }] }), { status: 200 }));
    fireEvent.change(screen.getByRole("textbox", { name: "Message ZominAI" }), { target: { value: "What did I ask you about?" } });
    fireEvent.click(screen.getByRole("button", { name: "Send message to ZominAI" }));
    expect(await screen.findByText("You asked for the date and time on this machine.")).toBeInTheDocument();
    const followUpRequest = JSON.parse(String(fetchSpy.mock.calls[2]?.[1]?.body)) as { messages: Array<{ content: string; role: string }> };
    expect(followUpRequest.messages).toEqual(expect.arrayContaining([
      expect.objectContaining({ role: "user", content: "What is the date and time on this machine?" }),
      expect.objectContaining({ role: "assistant", content: "The Zo Computer time is 15:30 UTC on 22 July 2026." }),
      expect.objectContaining({ role: "user", content: "What did I ask you about?" })
    ]));
    fireEvent.click(screen.getByRole("button", { name: "New ZominAI chat" }));
    fetchSpy.mockReset();
    ...
    fetchSpy.mockReset();
    fetchSpy.mockResolvedValueOnce(new Response(JSON.stringify({ choices: [{ message: { content: "guide.pdf is the largest file at 20 B." } }] }), { status: 200 }));
    fireEvent.change(screen.getByRole("textbox", { name: "Message ZominAI" }), { target: { value: "Tell me which file has the biggest file size." } });
    fireEvent.click(screen.getByRole("button", { name: "Send message to ZominAI" }));
    expect(await screen.findByText("guide.pdf is the largest file at 20 B.")).toBeInTheDocument();
    expect(client.list).toHaveBeenCalledWith({ prefix: undefined });
    const largestFileRequest = JSON.parse(String(fetchSpy.mock.calls[0]?.[1]?.body)) as { messages: Array<{ content: string; role: string }>; tools?: unknown[] };
    const largestFileContent = largestFileRequest.messages[0]?.content ?? "";
    expect(largestFileRequest.tools).toBeUndefined();
    expect(largestFileContent).toContain("current recursive Zo Drive file inventory");
    expect(largestFileContent).toContain('"largestFile":{"contentType":"application/pdf","key":"guide.pdf"');
    expect(largestFileContent).toContain('"totalBytes":35');
    expect(largestFileContent.indexOf('"key":"guide.pdf"')).toBeLessThan(largestFileContent.indexOf('"key":"photo.jpg"'));
    fireEvent.click(screen.getByRole("button", { name: "New ZominAI chat" }));
    fetchSpy.mockReset();
    fetchSpy
    ...
    fireEvent.click(screen.getByRole("button", { name: "New ZominAI chat" }));
    fetchSpy.mockReset();
    fetchSpy.mockImplementationOnce(async (_input, init) => await new Promise<Response>((_resolve, reject) => {
      init?.signal?.addEventListener("abort", () => reject(new DOMException("Stopped", "AbortError")), { once: true });
    }));
    fireEvent.change(screen.getByRole("textbox", { name: "Message ZominAI" }), { target: { value: "Keep thinking until I stop you" } });
    fireEvent.click(screen.getByRole("button", { name: "Send message to ZominAI" }));
    fireEvent.click(await screen.findByRole("button", { name: "Stop ZominAI response" }));
    expect(await screen.findByText("Response stopped.")).toBeInTheDocument();
    expect(screen.getAllByLabelText("ZominAI response metrics").at(-1)).toHaveTextContent("Stopped after");
    expect(screen.getByRole("button", { name: "Retry message to ZominAI" })).toBeInTheDocument();
    fireEvent.click(screen.getByRole("button", { name: "New ZominAI chat" }));
    ...
    fetchSpy.mockResolvedValue(new Response('event: zominai-error\ndata: {"error":{"code":"ZOMINAI_UNAVAILABLE","message":"The private Bonsai runtime is unavailable. Try again shortly."}}\n\n', { headers: { "content-type": "text/event-stream" }, status: 200 }));
    fireEvent.change(screen.getByRole("textbox", { name: "Message ZominAI" }), { target: { value: "Are you there?" } });
    fireEvent.click(screen.getByRole("button", { name: "Send message to ZominAI" }));
    expect(await screen.findByLabelText("ZominAI not ready")).toBeInTheDocument();
    ...
    fireEvent.click(screen.getByRole("button", { name: "Use Google Drive" }));
    expect(screen.getByTestId("drive-workspace")).toHaveAttribute("data-drive-theme", "google-drive");
    expect(window.localStorage.getItem("zo-drive:theme:v1")).toBe("google-drive");
    ...
    fireEvent.click(screen.getByText("remote-plan.pdf"));
    expect(await screen.findByRole("dialog", { name: "Preview remote-plan.pdf" })).toBeInTheDocument();
    await waitFor(() => expect(client.downloadClusterObject).toHaveBeenCalledWith({ id: "33333333-3333-4333-8333-333333333333", key: "remote-plan.pdf" }));
    fireEvent.click(screen.getByRole("button", { name: "Close preview" }));
    ...
    expect(screen.queryByRole("region", { name: "Links shared by you" })).not.toBeInTheDocument();
    fireEvent.click(screen.getByRole("tab", { name: "Share links" }));
    expect(await screen.findByRole("region", { name: "Links shared by you" })).toBeInTheDocument();
    expect(screen.queryByText("remote-plan.pdf")).not.toBeInTheDocument();
    ...
    fireEvent.click(screen.getByRole("button", { name: "Shared with others" }));
    fireEvent.click(await screen.findByRole("tab", { name: "Share links" }));
