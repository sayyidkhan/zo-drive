...
describe("DriveApp", () => {
  it("shows a public landing page with Drive and documentation entry points", () => {
    ...
    expect(screen.getByLabelText("Remote local-machine access").querySelector(".zo-remote-terminal")).toHaveTextContent("Ready for the next job");
  ...
  it("documents separate GUI and CLI workflows", () => {
    ...
    try {
      ...
      expect(screen.getByRole("heading", { name: "GUI version 1.42.26" })).toBeInTheDocument();
      ...
      expect(screen.getByRole("link", { name: "GUI releases version 1.42.26" })).toHaveAttribute("href", expect.stringContaining("?releases=1&mode=gui"));
      ...
      expect(screen.getAllByText(/zo-drive configure/).length).toBeGreaterThanOrEqual(1);
      ...
      expect(screen.getByText("Latest: v1.42.26")).toBeInTheDocument();
  ...
  it("shows storage usage, folders, and files supplied by the shared SDK", async () => {
    ...
    fireEvent.click(screen.getByRole("tab", { name: "Shared Folders" }));
