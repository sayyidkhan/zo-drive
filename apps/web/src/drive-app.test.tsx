...
describe("DriveApp", () => {
  ...
  it("documents separate GUI and CLI workflows", () => {
    ...
    try {
      ...
      expect(screen.getByRole("heading", { name: "GUI version 1.11.1" })).toBeInTheDocument();
      expect(screen.getByRole("link", { name: "Landing page" })).toHaveAttribute("href", "/");
      expect(screen.getByRole("link", { name: "GUI changelog version 1.11.1" })).toHaveAttribute("href", expect.stringContaining("?docs=1&mode=gui&page=changelog"));
      expect(screen.getByRole("heading", { name: "GUI changelog" })).toBeInTheDocument();
      expect(screen.getByText((_, element) => element?.tagName === "H3" && element.textContent === "GUI v1.11.1")).toBeInTheDocument();
      ...
      expect(screen.getByRole("heading", { name: "GUI changelog" })).toBeInTheDocument();
      expect(screen.getByText("Latest: v1.11.1")).toBeInTheDocument();
