...
function ApiKeys({ client }: { client: DriveClient }) {
  ...
  const createMutation = useMutation({
    mutationFn: () => client.createApiKey({ name: name.trim(), scopes: access === "write" ? ["read", "write"] : ["read"], expiresAt: expiry === "never" ? null : new Date(Date.now() + Number(expiry.replace("d", "")) * 24 * 60 * 60 * 1_000).toISOString() }),
    onSuccess: async (key) => {
  ...
  return <div className="max-w-5xl space-y-6"><section className="overflow-hidden rounded-2xl border border-slate-200 bg-white shadow-sm"><div className="bg-gradient-to-br from-slate-950 via-slate-900 to-blue-950 px-6 py-8 text-white"><span className="inline-flex items-center gap-2 rounded-full border border-cyan-200/20 bg-cyan-200/10 px-3 py-1 text-xs font-semibold uppercase tracking-[0.16em] text-cyan-100"><KeyRound size={14} /> Device access</span><h2 className="mt-4 text-3xl font-semibold tracking-tight">Scoped keys for your local machines.</h2><p className="mt-2 max-w-2xl text-sm leading-6 text-slate-300">Create one key per computer or automation. Keys are shown once, stored only as hashes, and can be revoked here without changing your Drive password.</p></div><div className="grid gap-5 p-5 md:grid-cols-[minmax(0,1fr)_15rem]"><div><label className="text-sm font-semibold text-slate-700">Key name<input aria-label="API key name" className="mt-2 w-full rounded-lg border border-slate-300 px-3 py-2.5 text-sm outline-none focus:border-blue-500 focus:ring-4 focus:ring-blue-100" placeholder="MacBook Pro" value={name} onChange={(event) => setName(event.target.value)} /></label><div className="mt-4"><p className="text-sm font-semibold text-slate-700">Access</p><div className="mt-2 grid gap-2 sm:grid-cols-2"><button className={`rounded-lg border p-3 text-left text-sm ${access === "write" ? "border-blue-500 bg-blue-50 text-blue-800" : "border-slate-200 text-slate-600"}`} onClick={() => setAccess("write")} type="button"><strong className="block">Read & write</strong><span className="mt-1 block text-xs">Upload, edit, download, and list.</span></button><button className={`rounded-lg border p-3 text-left text-sm ${access === "read" ? "border-blue-500 bg-blue-50 text-blue-800" : "border-slate-200 text-slate-600"}`} onClick={() => setAccess("read")} type="button"><strong className="block">Read only</strong><span className="mt-1 block text-xs">List and download only.</span></button></div></div></div><label className="text-sm font-semibold text-slate-700">Expires<select aria-label="API key expiry" className="mt-2 w-full rounded-lg border border-slate-300 bg-white px-3 py-2.5 text-sm" value={expiry} onChange={(event) => setExpiry(event.target.value)}><option value="30d">30 days</option><option value="90d">90 days</option><option value="365d">1 year</option><option value="never">Never</option></select><button className="mt-5 w-full rounded-lg bg-blue-600 px-4 py-2.5 text-sm font-semibold text-white hover:bg-blue-700 disabled:bg-slate-300" disabled={!name.trim() || createMutation.isPending} onClick={() => createMutation.mutate()}>{createMutation.isPending ? "Creating…" : "Create API key"}</button></label></div></section>{created && <section className="rounded-2xl border border-amber-200 bg-amber-50 p-5"><div className="flex gap-3"><ShieldAlert className="mt-0.5 shrink-0 text-amber-700" size={20} /><div className="min-w-0 flex-1"><h2 className="font-semibold text-amber-950">Copy this key now</h2><p className="mt-1 text-sm leading-6 text-amber-900">This is the only time Zo Drive can show it. Store it in your machine's secret manager or environment, never in source code.</p><div className="mt-4 flex gap-2"><code className="min-w-0 flex-1 overflow-x-auto rounded-lg border border-amber-200 bg-white px-3 py-2.5 text-xs text-slate-800">{created}</code><button aria-label="Copy API key" className="rounded-lg border border-amber-300 bg-white px-3 text-amber-800 hover:bg-amber-100" onClick={() => void copyText(created, "API key copied") }><Copy size={18} /></button></div></div></div></section>}<section className="rounded-2xl border border-slate-200 bg-white shadow-sm"><div className="border-b border-slate-100 px-5 py-4"><h2 className="font-semibold text-slate-900">Active keys</h2><p className="mt-1 text-sm text-slate-500">Revoke a key immediately if a device is lost or an automation is retired.</p></div>{keysQuery.isPending ? <p className="p-6 text-sm text-slate-500">Loading API keys…</p> : keys.length === 0 ? <p className="p-6 text-sm text-slate-500">No API keys yet.</p> : <div className="divide-y divide-slate-100">{keys.map((key) => <ApiKeyRow key={key.id} apiKey={key} onRevoke={() => { if (window.confirm(`Revoke ${key.name}? This cannot be undone.`)) revokeMutation.mutate(key.id); }} />)}</div>}</section></div>;
}
