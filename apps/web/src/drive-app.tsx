  ...
  return <div className="space-y-5">
    ...
    <section className="min-w-0 overflow-hidden rounded-2xl border border-slate-200 bg-white shadow-sm">
      <div className="flex flex-wrap items-center justify-between gap-3 border-b border-slate-100 bg-slate-50 px-5 py-4">
        ...
        {workspaceTab === "editor" && <div className="flex gap-2"><button className="rounded-lg border border-slate-300 px-3 py-2 text-sm font-semibold text-slate-700 hover:bg-white disabled:text-slate-400" disabled={!selectedId || deleteMutation.isPending} onClick={() => { if (window.confirm(`Delete ${name}?`)) deleteMutation.mutate(); }}>Delete</button><button className="rounded-lg bg-blue-600 px-4 py-2 text-sm font-semibold text-white hover:bg-blue-700 disabled:bg-slate-300" disabled={!name.trim() || !source.trim() || saveMutation.isPending} onClick={() => saveMutation.mutate()}>{saveMutation.isPending ? "Saving…" : selected ? "Save changes" : "Create function"}</button></div>}
      </div>
...
function Databases({ client }: { client: DriveClient }) {
  ...
  {!showWorkspace ? <DatabaseCatalog databaseCount={databases.length} engineStates={enginesQuery.data ?? []} installingEngine={installEngineMutation.isPending ? installEngineMutation.variables : null} updatingEngine={updateEngineMutation.isPending ? updateEngineMutation.variables : null} onCreateDatabase={(engine) => { setSelectedEngine(engine); setSelectedId(null); setDatabaseView("instances"); }} onInstallEngine={(engine) => installEngineMutation.mutate(engine)} onUpdateEngine={(engine) => updateEngineMutation.mutate(engine)} onViewDatabases={() => { setSelectedId(null); setDatabaseView("instances"); }} /> : <div className={`mt-8 ${activeDatabase ? "" : "grid gap-5 md:grid-cols-[15rem_minmax(0,1fr)]"}`}>
