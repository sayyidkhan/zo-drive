...
function DocsPage({ mode }: { mode: "gui" | "cli" }) {
  ...
  const cliConnect = `export ZO_DRIVE_API_URL="${serverUrl}"\nzo-drive login --username <username> --password '<password>'`;
  ...
  const sections = isGui
    ...
    : [
        ...
        { id: "connection", eyebrow: "Cloud connection", icon: <Cloud size={20} />, title: "Connect to your Zo Drive cloud", body: "Set your private Drive address once in the terminal, then authenticate that machine with your owner account.", steps: ["Export ZO_DRIVE_API_URL with the exact Zo Drive cloud address shown below.", "Run zo-drive login with your owner username and password.", "Copy the printed ZO_DRIVE_SESSION_TOKEN export into the same terminal session. Keep it private."] },
        { id: "cli", eyebrow: "Command line", icon: <Terminal size={20} />, title: "Upload with zo-drive", body: "Once connected, send files directly to your private Drive.", steps: ["Keep ZO_DRIVE_API_URL and ZO_DRIVE_SESSION_TOKEN available in the terminal session.", "Run zo-drive upload with a local file path and optional destination folder.", "Use zo-drive ls, download, delete, mkdir, and usage against the same private Drive."] },
        { id: "updates", eyebrow: "Updates", icon: <RefreshCw size={20} />, title: "Keep zo-drive current", body: "The global zo-drive command stays linked to this checkout. Pull and rebuild when updates are released; you do not need to install it again.", steps: ["Use the main branch when you want the latest changes as they are pushed.", "Use a Git release tag when you want a fixed, reproducible version on a machine.", "After switching versions, install dependencies and rebuild before running zo-drive again."] },
