...
const GUI_VERSION = "1.24.1";
const CLI_VERSION = "1.2.1";
const ZOMINAI_VERSION = "1.0.0";
...
function DriveModeSwitch({ guiHref = docsUrl("gui"), includeZominAi = false, mode, page = "docs" }: { guiHref?: string; includeZominAi?: boolean; mode: "gui" | "cli" | "zominai"; page?: "docs" | "changelog" }) {
  return <nav aria-label="Choose Zo Drive mode" className="inline-flex items-center rounded-lg border border-slate-200 bg-white p-0.5 shadow-sm">
    ...
    {includeZominAi && <a aria-current={mode === "zominai" ? "page" : undefined} className={`inline-flex items-center gap-1.5 rounded-md px-2.5 py-1 text-sm font-semibold transition ${mode === "zominai" ? "bg-cyan-950 text-white shadow-sm" : "text-slate-600 hover:bg-slate-100 hover:text-slate-950"}`} href={docsUrl("zominai", page)}><Cpu size={16} /> ZominAI</a>}
  </nav>;
}
...
function DocsPage({ mode, page }: { mode: "gui" | "cli" | "zominai"; page: "docs" | "changelog" }) {
  if (mode === "zominai") return page === "changelog" ? <ZominAiDocsChangelogPage /> : <ZominAiDocsPage />;
...
function ZominAiDocsModeSwitch({ page = "docs" }: { page?: "docs" | "changelog" }) {
  return <DriveModeSwitch includeZominAi mode="zominai" page={page} />;
}
...
function docsUrl(mode: "gui" | "cli" | "zominai" = "gui", page: "docs" | "changelog" = "docs"): string {
