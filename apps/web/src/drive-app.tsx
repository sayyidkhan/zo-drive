...
import { AuditTrailScreen, type AuditClient } from "./features/audit/audit-trail.js";
...
type DriveClient = Pick<ZoDriveClient, "createApiKey" | "createFolder" | "createNativeFile" | "createShare" | "delete" | "download" | "emptyTrash" | "getUsage" | "list" | "listApiKeys" | "listFolders" | "listFormResponses" | "listShares" | "listStarred" | "listTrash" | "permanentlyDeleteTrash" | "publishForm" | "rename" | "restoreTrash" | "revokeApiKey" | "revokeShare" | "saveNativeFile" | "setQuota" | "star" | "unstar" | "updateSharePasscode" | "upload"> & Partial<Pick<ZoDriveClient, "createClusterFolder" | "createClusterInvitation" | "createClusterMount" | "deleteClusterInvitation" | "deleteClusterMount" | "deleteClusterObject" | "deleteClusterPeer" | "downloadClusterObject" | "endDemoSessions" | "getClusterMountAccess" | "listAuditEvents" | "listClusterInvitations" | "listClusterMounts" | "listClusterObjects" | "listClusterPeers" | "renameClusterObject" | "resetDemoSandbox" | "updateClusterPeerRole" | "uploadClusterObject" | "createDatabase" | "createDatabaseApiKey" | "deleteDatabase" | "executeDatabase" | "exportDatabase" | "getDatabaseImportSettings" | "importDatabase" | "installDatabaseEngine" | "listDatabaseApiKeys" | "listDatabaseEngines" | "listDatabases" | "listDatabaseRows" | "listDatabaseTables" | "queryDatabase" | "revokeDatabaseApiKey" | "setDatabaseImportLimit" | "updateDatabaseEngine" | "createFunction" | "deleteFunction" | "listFunctions" | "listFunctionRuns" | "runFunction" | "updateFunction" | "deleteFolder" | "renameFolder">>;
...
type DriveSection = "api-keys" | "audit" | "cluster-databases" | "databases" | "demo-mode" | "functions" | "home" | "my-drive" | "pastes" | "profile" | "shared" | "starred" | "theme" | "transfer" | "trash" | "user-access" | "zominai";
...
const driveSections: DriveSection[] = ["api-keys", "audit", "cluster-databases", "databases", "demo-mode", "functions", "home", "my-drive", "pastes", "profile", "shared", "starred", "theme", "transfer", "trash", "user-access", "zominai"];
    ...
    <div className="relative isolate" ref={heroRef}>
      ...
      <section className="relative z-10 mx-auto grid max-w-7xl items-center gap-12 px-5 pb-20 pt-16 sm:px-8 lg:grid-cols-[1.05fr_.95fr] lg:pb-32 lg:pt-24">
        <div>
          ...
          <div className="mt-10 flex flex-wrap gap-x-6 gap-y-3 text-sm font-medium text-slate-600 lg:flex-nowrap"><span className="inline-flex items-center gap-2 whitespace-nowrap"><Check size={16} className="text-blue-600" /> Data stays on your Zo</span><span className="inline-flex items-center gap-2 whitespace-nowrap"><Check size={16} className="text-blue-600" /> Folder-preserving uploads</span><span className="inline-flex items-center gap-2 whitespace-nowrap"><Check size={16} className="text-blue-600" /> GUI, CLI &amp; MCP access</span></div>
        </div>
...
function DriveInterfacesSection() {
  return <section aria-label="Zo Drive interfaces" className="mx-auto max-w-7xl px-5 py-16 sm:px-8">
    <div className="flex flex-col justify-between gap-5 lg:flex-row lg:items-end">
      <div><p className="text-sm font-bold uppercase tracking-[0.15em] text-blue-600">One Drive. Three ways to work.</p><h2 className="mt-3 max-w-3xl text-3xl font-semibold tracking-tight text-slate-950 sm:text-4xl">Manage visually. Automate precisely. Connect agents directly.</h2></div>
      <p className="max-w-xl text-sm leading-6 text-slate-600">GUI, CLI and MCP use the same private Drive and scoped device-key boundary. Choose the interface that fits the work.</p>
    </div>
    <div className="mt-8 grid overflow-hidden rounded-[1.75rem] border border-slate-200 bg-white shadow-xl shadow-slate-900/5 lg:grid-cols-3">
      <article className="p-6 sm:p-8">
        <div className="flex items-center justify-between"><span className="grid size-10 place-items-center rounded-xl bg-blue-50 text-blue-700"><MonitorUp size={19} /></span><span className="font-mono text-xs font-semibold text-slate-400">01 / GUI</span></div>
        <h3 className="mt-9 text-2xl font-semibold tracking-tight text-slate-950">Work in context.</h3>
        <p className="mt-3 text-sm leading-6 text-slate-600">Browse, upload, share and manage all six products visually, including browser-only ZominAI.</p>
        <div className="mt-7 grid grid-cols-3 gap-px overflow-hidden rounded-xl bg-slate-200 text-center text-xs font-semibold text-slate-600"><span className="bg-slate-50 px-3 py-4">Upload</span><span className="bg-slate-50 px-3 py-4">Organise</span><span className="bg-slate-50 px-3 py-4">Share</span></div>
        <a className="mt-6 inline-flex items-center gap-1.5 text-sm font-bold text-blue-700 hover:text-blue-900" href={docsUrl()}>Explore GUI <ArrowUpRight size={15} /></a>
      </article>
      <article className="border-t border-white/10 bg-slate-950 p-6 text-white lg:border-l lg:border-t-0 sm:p-8">
        <div className="flex items-center justify-between"><span className="grid size-10 place-items-center rounded-xl bg-white/10 text-cyan-300"><Terminal size={19} /></span><span className="font-mono text-xs font-semibold text-slate-500">02 / CLI</span></div>
        <h3 className="mt-9 text-2xl font-semibold tracking-tight">Run repeatable work.</h3>
        <p className="mt-3 text-sm leading-6 text-slate-300">Install the CLI for scripts, bulk file transfer and direct terminal workflows.</p>
        <div className="mt-7 overflow-hidden rounded-xl border border-white/10 bg-[#070b1d] p-4 font-mono text-xs leading-6 text-slate-200"><p><span className="text-slate-500">$ </span><span className="text-cyan-300">zo-drive</span> upload ./plan.pdf</p><p className="mt-3 text-emerald-300">✓ Uploaded to Zo Drive</p></div>
        <a className="mt-6 inline-flex items-center gap-1.5 text-sm font-bold text-cyan-300 hover:text-white" href={docsUrl("cli")}>Explore CLI <ArrowUpRight size={15} /></a>
      </article>
      <article className="border-t border-emerald-800/40 bg-[#082f2a] p-6 text-white lg:border-l lg:border-t-0 sm:p-8">
        <div className="flex items-center justify-between"><span className="grid size-10 place-items-center rounded-xl bg-emerald-300/10 text-emerald-200"><Network size={19} /></span><span className="font-mono text-xs font-semibold text-emerald-100/45">03 / MCP</span></div>
        <h3 className="mt-9 text-2xl font-semibold tracking-tight">Give agents a Drive.</h3>
        <p className="mt-3 text-sm leading-6 text-emerald-50/75">Connect Codex, Claude or Cursor directly. No Zo Drive CLI install, with read and write scopes enforced per tool.</p>
        <div className="mt-7 overflow-hidden rounded-xl border border-emerald-200/15 bg-black/25 p-4 font-mono text-xs leading-6 text-emerald-50/80"><p className="text-emerald-200">Streamable HTTP</p><p className="truncate">{`${window.location.origin}${appBasePath}/mcp`}</p><p className="mt-2 text-emerald-100/50">Bearer zdk_••••••••</p></div>
        <a className="mt-6 inline-flex items-center gap-1.5 text-sm font-bold text-emerald-200 hover:text-white" href={`${docsUrl()}#mcp`}>Explore MCP <ArrowUpRight size={15} /></a>
      </article>
    </div>
  </section>;
}
...
function OwnershipAdvantage() {
  const benefits = [
    ...
    { number: "02", icon: <MonitorUp size={21} />, title: "Your machine, every workflow", body: "Use the browser, command line, MCP or TypeScript SDK to work with the same private storage." },
...
function LandingProductVideo() {
  return <section aria-label="Zo Drive product video" className="border-y border-slate-200 bg-white py-16 sm:py-20"><div className="mx-auto grid max-w-7xl gap-8 px-5 sm:px-8 lg:grid-cols-[minmax(0,.58fr)_minmax(0,1fr)] lg:items-center"><div><p className="text-sm font-bold uppercase tracking-[0.15em] text-blue-600">The product, in motion</p><h2 className="mt-3 max-w-md text-3xl font-semibold tracking-tight text-slate-950 sm:text-4xl">See Zo Drive in 58 seconds.</h2><p className="mt-4 max-w-md text-sm leading-6 text-slate-600">A quick walkthrough of the private workspace and the six focused tools built around it.</p><p className="mt-5 text-xs font-medium text-slate-400">Optional viewing · sound on</p></div><div className="overflow-hidden rounded-2xl border border-slate-200 bg-slate-950 shadow-xl shadow-slate-900/10"><iframe allow="accelerometer; autoplay; clipboard-write; encrypted-media; gyroscope; picture-in-picture; web-share" allowFullScreen aria-label="Watch Zo Drive in 58 seconds" className="block aspect-video w-full" referrerPolicy="strict-origin-when-cross-origin" src="https://www.youtube-nocookie.com/embed/YzoNZ2LS8Eg?rel=0" title="Zo Drive product walkthrough" /></div></div></section>;
}
...
function DocsGuidesPage({ mode }: { mode: "gui" | "cli" }) {
  ...
  const sections = isGui
    ? [
        ...
        { id: "mcp", eyebrow: "Agent access", icon: <Network size={20} />, title: "Connect Codex, Claude, or Cursor without installing the CLI", body: "Zo Drive exposes a remote Streamable HTTP MCP endpoint backed by the same private storage and scoped device keys as the GUI and CLI.", steps: ["Create a named device key under Zo Drive > API Keys.", "Choose read-only access for browsing, or add write scope only when the agent should change files.", "Add the endpoint and Bearer key to your MCP client. Permanent deletion is never exposed through MCP."] },
    ...
    <div className="w-full space-y-6">
      <section className="overflow-hidden rounded-2xl border border-slate-200 bg-white shadow-sm">
        ...
        <div className="space-y-5 p-5">
          {user.isDemo && <div className="rounded-xl border border-amber-200 bg-amber-50 p-4 text-sm leading-6 text-amber-900"><strong>Demo account:</strong> this writable sandbox is isolated from production data and can be reset by a super user.</div>}
...
function UserAccessScreen({ client, currentUser }: { client: UserAccessClient; currentUser: DriveUser }) {
  ...
  const createMutation = useMutation({
    mutationFn: () => client.createAccountMember({ username, password, access: isDemo ? "write" : access, role: isDemo ? "regular" : role, isDemo }),
    onSuccess: async () => {
  ...
  return <div className="w-full space-y-6">
    <section className="overflow-hidden rounded-2xl border border-slate-200 bg-white shadow-sm">
      ...
      <div role="alert" className="mx-5 mt-5 flex gap-3 rounded-xl border border-amber-200 bg-amber-50 p-4 text-amber-900"><ShieldAlert className="mt-0.5 shrink-0 text-amber-600" size={19} /><div><p className="text-sm font-semibold">Demo credentials are public</p><p className="mt-1 text-sm leading-6">While Demo Mode is on, its username and password appear on the sign-in page. Visitors can test writes only inside the isolated 1 GB sandbox; production data is never exposed.</p></div></div>
      <div className="grid gap-6 p-5 xl:grid-cols-[minmax(0,1fr)_22rem]">
        ...
        <form className="self-start rounded-xl border border-slate-200 bg-slate-50 p-5" onSubmit={(event) => { event.preventDefault(); createMutation.mutate(); }}><div className="flex items-center gap-2 text-slate-900"><Plus size={18} className="text-blue-600" /><h3 className="font-semibold">Add user</h3></div><p className="mt-1 text-sm leading-5 text-slate-500">Create a separate sign-in for this account.</p><label className={`mt-5 flex items-start gap-3 rounded-lg border bg-white p-3 ${hasDemoAccount ? "border-slate-200 text-slate-400" : "border-amber-200 text-slate-700"}`}><input aria-label="Demo account" checked={isDemo} className="mt-0.5 size-4 accent-amber-600" disabled={hasDemoAccount} onChange={(event) => setIsDemo(event.target.checked)} type="checkbox" /><span><span className="block text-sm font-semibold">Demo account</span><span className="mt-1 block text-xs leading-5">{hasDemoAccount ? "Remove the existing demo account before creating another." : "Show these credentials publicly only while Demo Mode is on."}</span></span></label><label className="mt-4 block text-sm font-medium text-slate-700">Username<input aria-label="New user username" className="mt-1.5 w-full rounded-lg border border-slate-300 bg-white px-3 py-2.5 outline-none focus:border-blue-500 focus:ring-4 focus:ring-blue-100" minLength={3} maxLength={32} onChange={(event) => setUsername(event.target.value)} required value={username} /></label><label className="mt-4 block text-sm font-medium text-slate-700">{isDemo ? "Public demo password" : "Temporary password"}<input aria-label="New user password" className="mt-1.5 w-full rounded-lg border border-slate-300 bg-white px-3 py-2.5 outline-none focus:border-blue-500 focus:ring-4 focus:ring-blue-100" minLength={6} onChange={(event) => setPassword(event.target.value)} required type={isDemo ? "text" : "password"} value={password} /></label>{isDemo && <p className="mt-2 text-xs leading-5 text-amber-700">This password will be visible to every visitor while Demo Mode is enabled.</p>}<label className="mt-4 block text-sm font-medium text-slate-700">Drive access<select aria-label="New user access" className="mt-1.5 w-full rounded-lg border border-slate-300 bg-white px-3 py-2.5 outline-none focus:border-blue-500 focus:ring-4 focus:ring-blue-100 disabled:bg-slate-100 disabled:text-slate-400" disabled={isDemo} onChange={(event) => setAccess(event.target.value as AccountAccess)} value={isDemo ? "write" : access}><option value="read">Read only</option><option value="write">Read & write</option></select></label><label className="mt-4 block text-sm font-medium text-slate-700">Account role<select aria-label="New user role" className="mt-1.5 w-full rounded-lg border border-slate-300 bg-white px-3 py-2.5 outline-none focus:border-blue-500 focus:ring-4 focus:ring-blue-100 disabled:bg-slate-100 disabled:text-slate-400" disabled={isDemo} onChange={(event) => setRole(event.target.value as AccountRole)} value={isDemo ? "regular" : role}><option value="regular">Regular user</option><option value="super">Super user</option></select></label><button className="mt-5 inline-flex w-full items-center justify-center gap-2 rounded-lg bg-blue-600 px-4 py-2.5 text-sm font-semibold text-white hover:bg-blue-700 disabled:bg-slate-300" disabled={createMutation.isPending} type="submit"><Plus size={17} /> {createMutation.isPending ? "Adding user…" : "Add user"}</button></form>
      </div>
    </section>
  </div>;
}
...
function DriveScreen({ authClient, client, user, onAccountDeleted, onSignOut }: { authClient: AuthClient; client: DriveClient; user: DriveUser; onAccountDeleted: () => void; onSignOut: () => void }) {
  ...
  useEffect(() => {
    if ((section === "user-access" || section === "demo-mode" || section === "audit") && !canManageUserAccess) {
      setSection("my-drive");
      setCurrentPath("");
    }
        ...
        <div data-testid="header-actions" className="order-2 ml-auto flex shrink-0 items-center gap-0.5 text-sm font-medium text-slate-500 md:order-none md:ml-0 md:gap-1">
          ...
          <div className="relative">
            ...
            {accountMenuOpen && <div className="absolute right-0 top-11 z-20 w-52 rounded-xl border border-slate-200 bg-white p-1.5 shadow-lg">
              ...
              {canManageUserAccess && <button className="flex w-full items-center gap-2 rounded-lg px-3 py-2 text-left text-sm font-medium text-slate-700 hover:bg-slate-100" onClick={() => { setAccountMenuOpen(false); setSection("audit"); setCurrentPath(""); }}><ScrollText size={17} /> Audit trail</button>}
        ...
        <section className="min-h-0 min-w-0 max-w-full flex-1 overflow-x-hidden overflow-y-auto p-4 transition-[padding] duration-500 ease-[cubic-bezier(0.22,1,0.36,1)] motion-reduce:transition-none sm:p-6 md:p-9">
          <div className="mb-7 flex flex-wrap items-center justify-between gap-4">
            <div>
              ...
              {!showingSearchResults && section === "audit" && <p className="mt-1 text-sm text-slate-500">Review account sign-ins and security-relevant actions.</p>}
...
function Databases({ client, isDemo, search }: { client: DriveClient; isDemo: boolean; search: string }) {
  const queryClient = useQueryClient();
  ...
  {!showWorkspace ? <DatabaseCatalog engineStates={enginesQuery.data ?? []} installingEngine={installEngineMutation.isPending ? installEngineMutation.variables : null} updatingEngine={updateEngineMutation.isPending ? updateEngineMutation.variables : null} onCreateDatabase={(engine) => { setSelectedEngine(engine); setSelectedId(null); setDatabaseView("instances"); }} onInstallEngine={(engine) => installEngineMutation.mutate(engine)} onUpdateEngine={(engine) => updateEngineMutation.mutate(engine)} /> : <div className={`mt-8 ${activeDatabase ? "" : "grid gap-5 md:grid-cols-[15rem_minmax(0,1fr)]"}`}>
    ...
    <div className="min-w-0">
      {!activeDatabase ? <section className="grid min-h-96 place-items-center rounded-2xl border border-dashed border-slate-300 bg-white p-8 text-center"><div><span className="mx-auto grid size-12 place-items-center rounded-2xl bg-blue-50 text-blue-600"><Database size={24} /></span><h2 className="mt-4 text-xl font-semibold text-slate-900">Choose a database to open its workspace.</h2><p className="mt-2 max-w-sm text-sm leading-6 text-slate-500">Each database opens in its own full-width screen, with its own data tools and backend access.</p></div></section> : <section className="overflow-hidden rounded-2xl border border-slate-200 bg-white shadow-sm">
        ...
        {activePanel === "access" && <div className="p-4 sm:p-6"><DatabaseConnection client={client} database={activeDatabase} isDemo={isDemo} /></div>}
      </section>}
    </div>
...
function CredentialUnavailable({ label }: { label: string }) {
  return <section className="rounded-2xl border border-amber-200 bg-amber-50 p-6 shadow-sm"><LockKeyhole className="text-amber-700" size={24} /><h2 className="mt-4 text-lg font-semibold text-amber-950">{label} is not available in Demo Mode</h2><p className="mt-2 max-w-xl text-sm leading-6 text-amber-800">This feature is available in regular Zo Drive accounts. Demo Mode does not request, create, list, expose, or revoke credential data.</p></section>;
}

function DatabaseConnection({ client, database, isDemo }: { client: DriveClient; database: DriveDatabase; isDemo: boolean }) {
  if (isDemo) return <CredentialUnavailable label="Database credentials" />;
  return <DatabaseConnectionAvailable client={client} database={database} />;
}

function DatabaseConnectionAvailable({ client, database }: { client: DriveClient; database: DriveDatabase }) {
  const queryClient = useQueryClient();
...
function ApiKeys({ client, isDemo }: { client: DriveClient; isDemo: boolean }) {
  if (isDemo) return <CredentialUnavailable label="API Keys" />;
  return <ApiKeysAvailable client={client} />;
}

function ApiKeysAvailable({ client }: { client: DriveClient }) {
  const queryClient = useQueryClient();
  const [name, setName] = useState("");
