...
function DocsPage({ mode, page }: { mode: "gui" | "cli"; page: "docs" | "changelog" }) {
  return page === "changelog" ? <DocsChangelogPage mode={mode} /> : <><DocsChangelogLink mode={mode} /><DocsGuidesPage mode={mode} /></>;
}

function DocsChangelogLink({ mode }: { mode: "gui" | "cli" }) {
  const version = mode === "gui" ? GUI_VERSION : CLI_VERSION;
  const label = mode === "gui" ? "GUI" : "CLI";
  return <a aria-label={`${label} changelog version ${version}`} className="fixed right-5 top-3 z-30 inline-flex items-center gap-2 rounded-lg border border-slate-200 bg-white px-3 py-2 text-sm font-semibold text-slate-700 shadow-sm transition hover:border-blue-200 hover:bg-blue-50 hover:text-blue-700 sm:right-8" href={docsUrl(mode, "changelog")}><ScrollText size={16} /> Changelog <span className="rounded bg-slate-100 px-1.5 py-0.5 text-xs font-bold text-slate-600">v{version}</span></a>;
}
      ...
      <div className="flex min-h-[calc(100vh-4.5rem)]">
        <aside id="drive-navigation" className={`${sidebarOpen ? "w-64 overflow-hidden px-3" : "w-16 overflow-visible px-1.5"} shrink-0 border-r border-slate-200 bg-white py-5 transition-[width,padding] duration-200`}>
          ...
          <nav className={`${sidebarOpen ? "mt-6 space-y-1" : "mt-5 space-y-2"}`}>
            ...
            <button aria-label="Zo Cluster Databases" className={`flex items-center rounded-lg text-sm font-semibold ${sidebarOpen ? "w-full gap-3 px-3 py-2.5 text-left" : "mx-auto size-10 justify-center"} ${section === "cluster-databases" ? "bg-blue-50 text-blue-700" : "text-slate-600 hover:bg-slate-50"} ${collapsedNavigationTooltip(sidebarOpen)}`} data-tooltip="Zo Cluster Databases" onClick={() => { setSection("cluster-databases"); setCurrentPath(""); }} title="Zo Cluster Databases"><Network size={18} />{sidebarOpen && <span>Zo Cluster Databases</span>}</button>
          </nav>
        ...
        <section className="min-w-0 flex-1 p-6 md:p-9">
          <div className="mb-7 flex flex-wrap items-center justify-between gap-4">
            <div>
              ...
              {section === "cluster-databases" && <p className="mt-1 text-sm text-slate-500">Connect trusted Zo Computers through private, folder-scoped shared storage.</p>}
...
function Functions({ client }: { client: DriveClient }) {
  ...
  return <div className="space-y-5">
    ...
    {selectedId && <FunctionLogs isLoading={runsQuery.isPending} runs={runs} />}
  </div>;
}
...
function ClusterDatabases({ client }: { client: DriveClient }) {
  const queryClient = useQueryClient();
  const supported = Boolean(client.createClusterInvitation && client.createClusterMount && client.listClusterMounts && client.listClusterObjects);
  ...
  const invitationMutation = useMutation({ mutationFn: () => client.createClusterInvitation!(folder.trim()), onSuccess: (invite) => { setCreatedInvite(invite.token); toast.success("One-time pairing key created"); }, onError: (error) => toast.error(error instanceof Error ? error.message : "Could not create pairing key") });
  ...
  return <div className="space-y-5">
    <section className="relative overflow-hidden rounded-2xl bg-gradient-to-br from-slate-950 via-cyan-950 to-slate-900 px-7 py-8 text-white shadow-sm md:px-9">
      ...
      <div className="relative max-w-3xl">
        <span className="inline-flex items-center gap-2 rounded-full border border-cyan-100/20 bg-cyan-100/10 px-3 py-1 text-xs font-semibold uppercase tracking-[0.16em] text-cyan-100"><Network size={14} /> Private multi-Zo storage</span>
        <h2 className="mt-4 text-3xl font-semibold tracking-tight md:text-4xl">Share one folder. Keep every boundary deliberate.</h2>
        <p className="mt-3 max-w-2xl text-sm leading-6 text-slate-300">Zo Cluster Databases links trusted Zo Computers through an explicitly shared folder. The selected folder and every nested file become a live collaborative drive; nothing else in either Drive is exposed.</p>
      </div>
    </section>
    ...
    <section className="grid gap-5 lg:grid-cols-[minmax(0,1.45fr)_minmax(18rem,0.85fr)]">
      <div className="overflow-hidden rounded-2xl border border-slate-200 bg-white shadow-sm">
        <div className="flex flex-wrap items-start justify-between gap-4 border-b border-slate-100 bg-slate-50 px-5 py-5">
          <div><p className="text-xs font-bold uppercase tracking-[0.16em] text-cyan-700">Cluster connections</p><h2 className="mt-1 text-xl font-semibold tracking-tight text-slate-900">Pair a trusted Zo Computer</h2><p className="mt-2 max-w-xl text-sm leading-6 text-slate-500">Choose one folder, send its time-limited pairing key to the other Zo owner, then accept it from their Drive.</p></div>
          <span className="rounded-full bg-slate-200 px-3 py-1.5 text-xs font-bold text-slate-600">Private by default</span>
        </div>
        <div className="grid gap-4 p-5 sm:grid-cols-3">
          <ClusterStep number="1" title="Connect a Zo" description="Pair another Zo Computer using its private cluster identity." />
          <ClusterStep number="2" title="Choose a folder" description="Only the folder you approve, including its nested contents, is shared." />
          <ClusterStep number="3" title="Work together" description="Both parties see the same files and changes in their shared drive." />
        </div>
        <div className="grid gap-4 border-t border-slate-100 p-5 lg:grid-cols-2"><div><label className="text-sm font-semibold text-slate-700">Folder to share<input aria-label="Cluster folder" className="mt-1.5 w-full rounded-lg border border-slate-300 px-3 py-2.5 text-sm outline-none focus:border-cyan-600 focus:ring-4 focus:ring-cyan-100" placeholder="Projects/Shared" value={folder} onChange={(event) => setFolder(event.target.value)} /></label><button className="mt-3 rounded-lg bg-slate-900 px-4 py-2.5 text-sm font-semibold text-white hover:bg-slate-800 disabled:bg-slate-300" disabled={!folder.trim() || invitationMutation.isPending} onClick={() => invitationMutation.mutate()}>{invitationMutation.isPending ? "Creating…" : "Create pairing key"}</button>{createdInvite && <div className="mt-3 rounded-xl border border-cyan-200 bg-cyan-50 p-3"><p className="text-xs font-bold uppercase tracking-wide text-cyan-800">Send once</p><code className="mt-2 block break-all text-xs leading-5 text-cyan-950">{createdInvite}</code><button className="mt-2 text-xs font-semibold text-cyan-800 hover:text-cyan-950" onClick={() => void copyText(createdInvite, "Pairing key copied")}>Copy pairing key</button></div>}</div><div><label className="text-sm font-semibold text-slate-700">Other Zo Drive URL<input aria-label="Remote Zo Drive URL" className="mt-1.5 w-full rounded-lg border border-slate-300 px-3 py-2.5 text-sm outline-none focus:border-cyan-600 focus:ring-4 focus:ring-cyan-100" placeholder="https://other-drive.example/drive" value={remoteUrl} onChange={(event) => setRemoteUrl(event.target.value)} /></label><label className="mt-3 block text-sm font-semibold text-slate-700">Pairing key<input aria-label="Cluster pairing key" className="mt-1.5 w-full rounded-lg border border-slate-300 px-3 py-2.5 font-mono text-xs outline-none focus:border-cyan-600 focus:ring-4 focus:ring-cyan-100" value={inviteToken} onChange={(event) => setInviteToken(event.target.value)} /></label><button className="mt-3 rounded-lg bg-cyan-700 px-4 py-2.5 text-sm font-semibold text-white hover:bg-cyan-800 disabled:bg-slate-300" disabled={!remoteUrl.trim() || !inviteToken.trim() || mountMutation.isPending} onClick={() => mountMutation.mutate()}>{mountMutation.isPending ? "Connecting…" : "Connect shared folder"}</button></div></div>
      </div>
    ...
    {(mountsQuery.data ?? []).length > 0 && <section className="overflow-hidden rounded-2xl border border-slate-200 bg-white shadow-sm"><header className="border-b border-slate-100 px-5 py-4"><h2 className="font-semibold text-slate-900">Connected folders</h2><p className="mt-1 text-sm text-slate-500">Credentials remain on this Zo; the browser only sees the mounted folder.</p></header><div className="divide-y divide-slate-100">{mountsQuery.data?.map((mount) => <button className="flex w-full items-center gap-3 px-5 py-4 text-left hover:bg-slate-50" key={mount.id} onClick={() => setOpenMount(mount.id)}><span className="grid size-9 place-items-center rounded-lg bg-cyan-100 text-cyan-800"><Network size={17} /></span><span className="min-w-0 flex-1"><span className="block font-semibold text-slate-800">{mount.folder}</span><span className="block truncate text-xs text-slate-500">{mount.remoteUrl}</span></span></button>)}</div>{openMount && <div className="border-t border-slate-100 bg-slate-50 p-5"><p className="text-sm font-semibold text-slate-800">Shared files</p>{objectsQuery.isPending ? <p className="mt-2 text-sm text-slate-500">Loading remote folder…</p> : <div className="mt-3 space-y-2">{(objectsQuery.data ?? []).map((object) => <div className="rounded-lg border border-slate-200 bg-white px-3 py-2 text-sm text-slate-700" key={object.key}>{object.key}</div>)}{(objectsQuery.data ?? []).length === 0 && <p className="text-sm text-slate-500">No files in this shared folder yet.</p>}</div>}</div>}</section>}
  </div>;
}
