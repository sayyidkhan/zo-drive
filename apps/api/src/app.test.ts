...
describe("Zo Drive API", () => {
  ...
  it("streams authenticated ZominAI responses without buffering them", async () => {
    ...
    const runtimeFetch = vi.fn(async (_input: RequestInfo | URL, init?: RequestInit) => {
      expect(JSON.parse(String(init?.body))).toMatchObject({ model: "Bonsai-8B-Q1_0.gguf", stream: true, stream_options: { include_usage: true } });
    ...
    const response = await app.request("http://localhost/zominai/chat", { method: "POST", headers: { "content-type": "application/json", "x-test-user-id": "alice" }, body: JSON.stringify({ messages: [{ content: "Stream a reply", role: "user" }], stream: true, stream_options: { include_usage: true } }) });
